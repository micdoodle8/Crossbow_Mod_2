...
public class ItemCrossbow extends Item
{
	...
	public void onUpdate(ItemStack itemstack, World world, Entity entity, int i, boolean flag)
	{
		if (entity instanceof EntityPlayer && ((EntityPlayer) entity).inventory.getCurrentItem() == itemstack)
		{
        	...
        	if ((Util.hasHeavyMech(itemstack) || Util.hasMediumMech(itemstack) || Util.hasLightMech(itemstack)) && mouseHeld) // IS FULL AUTO
        	{
        		itemstack = shoot(itemstack, world, entityplayer);
        	}
        	else if (mouseHeld && !lastMouseHeld) // IS SEMI AUTO
        	{
        		itemstack = shoot(itemstack, world, entityplayer);
        	}
		}
